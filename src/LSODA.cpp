...
void LSODA::lsoda(LSODA_ODE_SYSTEM_TYPE f, const size_t neq, vector<double> &y, double *t,
    ...
    array<double, 4> &rworks, double data)
{
    assert(tout > *t);

    int mxstp0 = 500000, mxhnl0 = 10;
...
void LSODA::stoda(
    const size_t neq, vector<double> &y, LSODA_ODE_SYSTEM_TYPE f, double data)
{
    assert(neq + 1 == y.size());
    ...
    */
    while(1) {
        while(1) {
            ...
            correction(
                neq, y, f, &corflag, pnorm, &del, &delp, &told, &ncf, &rh, &m, data);
            if(corflag == 0)
                break;
            if(corflag == 1) {
...
void LSODA::correction(const size_t neq, vector<double> &y, LSODA_ODE_SYSTEM_TYPE f,
    ...
    double *rh, size_t *m, double data)
{
...
void LSODA::lsoda_update(LSODA_ODE_SYSTEM_TYPE f, const size_t neq, vector<double> &y,
    vector<double> &yout, double *t, const double tout, int *istate, double data,
